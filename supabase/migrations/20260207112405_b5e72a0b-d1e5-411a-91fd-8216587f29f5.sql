...
-- 2. RPC for admin queue state updates
...
-- 3. RPC for admin professional updates
...
-- 4. Remove permissive UPDATE policies on queue_state
DROP POLICY IF EXISTS "Public update access" ON public.queue_state;
DROP POLICY IF EXISTS "Authenticated users can update" ON public.queue_state;

-- 5. Remove permissive write policies on professionals
DROP POLICY IF EXISTS "Anyone can update professionals queue" ON public.professionals;
